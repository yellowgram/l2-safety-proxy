...
| **-32083** | `policy_denied` | **No. Non-retryable.** | **Yes.** Stop the loop | **Yes.** A human updates the allowlist or rejects the tool | Yes, if the destination is intended — then `policy:check`, restart, canary |
